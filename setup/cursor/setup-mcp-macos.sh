...
[ -f "$CONFIG_DIR/mcp-config.json" ] && CONFIG_FILE="$CONFIG_DIR/mcp-config.json"

echo "Setting up Yaade for Cursor (macOS)"
echo "===================================="
...
if [ "$USE_PIP_MODE" = "1" ]; then
    python3 "$PROJECT_ROOT/setup/mcp_config.py" write cursor "$CONFIG_FILE" 1 "$YAADE_PATH"
else
    python3 "$PROJECT_ROOT/setup/mcp_config.py" write cursor "$CONFIG_FILE" 0 "$UV_PATH" "$PROJECT_ROOT"
fi
